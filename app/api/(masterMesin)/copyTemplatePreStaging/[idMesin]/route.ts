...
function findDivisiTo(
    oldId: number,
    mapping: { id_divisi_from: number; id_divisi_to: number }[]
): number | null {
    const found = mapping.find((item) => item.id_divisi_from === oldId);
    ...
    try {
        ...
        if (!mesinExists) {
            return NextResponse.json(
                { success: false, message: "Mesin tidak ditemukan" },
                { status: 400 }
            );
        }
        ...
        if (!oldDivisi.length) {
            return NextResponse.json(
                { success: false, message: "Tidak ada template divisi untuk disalin" },
                { status: 400 }
            );
        }

        await prisma.mst_divisi.createMany({
            data: oldDivisi.map((divisi) => ({ name: divisi.name, id_mesin: idMesin })),
        });
        ...
        const divisiMapping = mapDivisi(
            oldDivisi.map((divisi) => divisi.id),
            newDivisi.map((divisi) => divisi.id)
        );
        ...
        const dataItems = oldChecklist
            .map((checklist) => {
                const targetDivisiId = findDivisiTo(
                    Number(checklist.id_divisi),
                    divisiMapping
                );
                if (!targetDivisiId) return null;

                return {
                    test_desc: checklist.test_desc,
                    result_detail: checklist.result_detail,
                    id_divisi: targetDivisiId,
                    id_mesin: idMesin,
                    id_type_values: checklist.id_type_values,
                };
            })
