...
export async function GET(_req: NextRequest, context: { params: Promise<{ idPoMaster: string; idCustomer: string; idGudang: string; dateFrom: string; dateTo: string }> }) {
    try {
        const { idPoMaster, idCustomer, idGudang, dateFrom, dateTo } = await context.params;
        const idPoMasterNum = toPositiveInt(idPoMaster);
        const idCustomerNum = toPositiveInt(idCustomer);
        const idGudangNum = toPositiveInt(idGudang);

        if (!idPoMasterNum || !idCustomerNum || !idGudangNum) return validationError({ id: ["Parameter id tidak valid"] });
