...
class PurchaseOrderController extends Controller
{
            ...
            if ($po){
                ...
                for ($i = 0; $i < count($dataArray); $i++) {
                     if ( !in_array($dataArray[$i], ['ATM_MESIN','CRM_MESIN', 'TCR_MESIN', 'CS_KIOS_MESIN', 'VBK_MESIN'])){
                         $sql .= $dataArray[$i]." varchar(50), ".$dataArray[$i]."_PN varchar(50),";
                     } else {
    ...
    function getDataImplementSummary(){
    ...
    function getWarehouseSummary($idWarehouse, $idCustomer, $idModel, $idStyle, $statusMesin, $process){
          $query = 'SELECT xx.*
                     FROM
        ...
        if ($idWarehouse != 'null'){
            $query .= ' and xx.id_warehouser = '.$idWarehouse; 
        }
