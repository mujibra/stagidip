...
class PurchaseOrderController extends Controller
{
    public function index($user_login){
        ...
        for ( $i = 0; $i < count($this->po); $i++){
            ...
            $getStatusPo    = MasterStatusPo::where('id', $this->po[$i]->id_status_po)->first();
            ...
            // get pengurangan untuk po Dummay
            // if ($this->po[$i]['status_po'] == 'Dummy'){
            if ($this->po[$i]['id_status_po'] == 1){
                $get_count_data_jml_keluar = PurchaseOrderHistoryCopyFromDummy::where('from_id_po', $this->po[$i]['id'])->count();
                $this->po[$i]['total_keluar'] = $get_count_data_jml_keluar;
            } else {
                $this->po[$i]['total_keluar'] = 0;
            }
            ...
            $this->po[$i]['status_po_details']  = $getStatusPo;
            $this->po[$i]['copy_from_po'] = PurchaseOrder::whereId($this->po[$i]['copy_from_id_po'])->first();

            if ($this->po[$i]['copy_from_po'] != null){
                $this->po[$i]['copy_from_po']['style'] = MasterStyle::where('id',  $this->po[$i]['copy_from_po']['style'])->first();
                $this->po[$i]['copy_from_po']['model'] = MasterMesin::where('id', $this->po[$i]['copy_from_po']['id_type_mesin'])->first();
            }

            $get_snMesin_from_dummy = isset($this->po[$i]['sn_mesins']) ? json_decode($this->po[$i]['sn_mesins'], true) : [];
            $this->po[$i]['sn_mesins'] =  $get_snMesin_from_dummy;
    ...
    function getDataImplementSummaryV2($idPoMaster, $idCustomer, $dateFrom, $dateTo){
        ...
        $where = '';
        if ($idPoMaster != 'null'){
            $where .= ' and tp.id_po_master = '.$idPoMaster; 
        }

        if ($idCustomer != 'null'){
            $where .= ' and tp.customer = '.$idCustomer; 
        }

        $query = 'select xx.* 
                ,round((xx.total_kirim * 100) / xx.quantity, 0) as persen_kirim
                from (
                    select 
                    -- tp.id as id_po, 
                    tp.id_po_master, 
                    -- tp.no_po,
                    case when tp.no_po like \'No%\' then 
                        trim(substr(tp.no_po, 4, length(tp.no_po)))
                    else 
                        tp.no_po
                    end no_po, 
                    tp.customer id_customer, 
                    ...
                    sum(tot_kirim.jml) as total_kirim
                    -- tot_kirim.jml
                    from tbl_po tp 
                    left outer join (
                        select tsd.id_po, count(*) as jml from transaksi_status_delivery tsd 
                    ...
                    (select id, name from models ) mdl, 
                    (select id, type from mst_mesin ) mesin
                    where 1=1 '.$where.' and mdl.id = tp.model
                    and mesin.id = tp.id_type_mesin
                    and exists ( select 1 from transaksi_inspeksi ti where ti.id_po = tp.id)
                    and tp.deleted_at is null
                    -- and tp.id_po_master = 43
                    -- and tp.id_po_master in (57, 63, 46)
                    group by 
                    tp.id_po_master,
                    -- tp.no_po,
                    case when tp.no_po like \'No%\' then 
                    ...
                    tp.customer, 
                    concat(mdl.name, "-", mesin.type), 
                    tp.model, 
                    tp.id_po_master,
                    tp.id_type_mesin
                ) xx                  
                order by xx.id_po_master';
           
           $get_data_summ = DB::select($query);

           for ($x=0; $x< count($get_data_summ); $x++){
                $check_po = PurchaseOrder::select('tbl_po.id', 'mdl.name')
                    ->join('models as mdl', 'tbl_po.model', '=', 'mdl.id')
                    ->where('tbl_po.id_po_master', $get_data_summ[$x]->id_po_master)
                    ->where('tbl_po.id_type_mesin', $get_data_summ[$x]->id_model)
                    ->where('tbl_po.deleted_at', '=', null)
                    ->get();
                
                $jml_mesin_activated = 0;

                for ($y=0; $y< count($check_po); $y++){

                    $column_snMesin = '';
                    switch ($check_po[$y]['name']){
                        case "ATM"      : $column_snMesin ='ATM_MESIN'; break;
                        case "ATMS"     : $column_snMesin ='ATM_MESIN'; break;
                        case "CRM"      : $column_snMesin ='CRM_MESIN'; break;
                        case "TCR"      : $column_snMesin ='TCR_MESIN'; break;
                    ...
                    $qry1 = 'select '.$column_snMesin.' from crt_'.$check_po[$y]['id']; //.$get_data_summ[$xx]->id_po;
                    $getSnMesin = DB::select($qry1);

                    for( $z=0; $z< count($getSnMesin); $z++ ){

                        if ($dateFrom != 'null' and $dateTo != 'null'){
                            
                            $get_data_aktivasi =  MasterWSInfo::where('serial_number', $getSnMesin[$z]->$column_snMesin)
                                                    ->where('installation_date', '>=', $dateFrom)
                                                    ->where('installation_date', '<=', $dateTo)
                                                    ->first();

                        } else if ($dateFrom != 'null' and $dateTo == 'null' ){
                            ...
                            $get_data_aktivasi =  MasterWSInfo::
                                        where('serial_number', $getSnMesin[$z]->$column_snMesin)->
                                        where('installation_date', '<>', null)->first();
                ...
                $get_data_summ[$x]->total_activated = $jml_mesin_activated;
                $get_data_summ[$x]->total_ba        = $jml_mesin_activated;
                $get_data_summ[$x]->persen_ba       = $jml_mesin_activated ? ($jml_mesin_activated * 100) / $jml_mesin_activated : 0; 
           }
    ...
    function getWarehouseSummary($idWarehouse, $idCustomer, $idModel, $idStyle, $statusMesin, $process, $dateFrom, $dateTo){
        ...
        $query = 'SELECT xx.id_po, 
                    ...
                    xx.qty_before_delivery, 
                    -- xx.qty_delivery + xx.qty_withdrawal as qty_delivery, 
                    -- xx.qty_delivery + ( xx.qty_request_delivery -  xx.qty_withdrawal ) as qty_delivery, 
                    -- xx.qty_delivery + (xx.qty_withdrawal + xx.qty_request_delivery ) as qty_delivery, 
                    -- case 
    ...
    // 2023-09-07 
    function getAllPoDummyBasedOnIdModel()
    {
        $datas = PurchaseOrder::
            select(['tbl_po.id', 'tbl_po.no_po', 'style.name', 'msn.type as model', 
            ...
            ->where('tbl_po.id_status_po', 1)->get();
    ...
    function getSnMesinByIdPoDummy($idPoDummay){
        $exists_po = PurchaseOrder::whereId($idPoDummay)->where('id_status_po', 1)->exists();
