...
function Row(props) {
  ...
  useEffect(() => {
    if (row?.checklistStaging[2]?.detail_staging?.fill_columns !== null) {
      // console.log('first');
      setselectedHour(
        row?.checklistStaging[2]?.detail_staging?.fill_columns[0]
      );
      setselectedMinute(
        row?.checklistStaging[2]?.detail_staging?.fill_columns[1]
      );
      setselectedSecond(
        row?.checklistStaging[2]?.detail_staging?.fill_columns[2]
      );
    }
    if (row?.checklistStaging[3]?.detail_staging?.fill_columns !== null) {
      setselectedHour2(
        row?.checklistStaging[3]?.detail_staging?.fill_columns[0]
      );
      setselectedMinute2(
        row?.checklistStaging[3]?.detail_staging?.fill_columns[1]
      );
      setselectedSecond2(
        row?.checklistStaging[3]?.detail_staging?.fill_columns[2]
      );
    }
    if (row?.checklistStaging[2]?.detail_staging?.fill_columns !== null) {
      ...
      setDevSelected4(
        row?.checklistStaging[2]?.detail_staging?.fill_columns[3]
      );
      setDevSelected5(
        row?.checklistStaging[2]?.detail_staging?.fill_columns[4]
      );
    }
    ...
    if (row?.checklistStaging[1]?.detail_staging?.fill_columns !== null) {
      setDisplaySelected(
        row?.checklistStaging[1]?.detail_staging?.fill_columns[0]
      );
      setDisplayVSelected(
        row?.checklistStaging[1]?.detail_staging?.fill_columns[1]
      );
    }
    const arr = [];
    if (getIdDivisi === 33) {
  ...
  // console.log(pageState, 'pageState11');
  const handleSubmit = () => {
    // getData();
    setLoadingBtn(true);
    axios
    ...
    <>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        <TableCell
          ...
          onClick={() => handleCloseTabble(props?.index)}
                          ...
                          {index === 0 ? (
                            ...
                            >
                              <>
                                ...
                                <img
                                  src={
                                    row?.checklistStaging[4]?.detail_staging
                                      ?.fill_columns
                                  }
                                  alt="Gambar Kosong"
                                />
...
export default function OpenMX8600SNew(props) {
