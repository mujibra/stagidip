...
function createData(id, unit, part_number, rev_final) {
  return { id, unit, part_number, rev_final };
}

export default function OpenExteriorCheck2(props) {
  ...
  const rows = data?.map((item, index) =>
    createData(item?.id, item?.unit, item?.part_number, item?.rev_final)
  );
