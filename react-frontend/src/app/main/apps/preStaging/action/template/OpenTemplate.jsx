...
function createData(id, name, test_desc, result_detail, detail_staging, item_type_values) {
  return { id, name, test_desc, result_detail, detail_staging, item_type_values };
}
...
export default function OpenTemplate(props) {
  ...
  const handleAllState = (e, newValue, index, multi = false, idxMulti = 0, count) => {
    if (pageState.length > 0) {
      const copyPageState = [...pageState];
      const insertState = copyPageState[index];
      const copyObject = { ...insertState };
  ...
  useEffect(() => {
    const arr = [];
    const arrBody = [];
    function createNullArray(start, end) {
      const length = end - start + 1;
      return length > 0 ? Array(length).fill(null) : null;
    }
    const arrNull = [];
    for (let index = 0; index < datas.length; index++) {
      arrNull.push(createNullArray(1, datas[index].item_type_values?.count_data));
    }
    // console.log(arrNull, 'arr');
    datas?.map((items, index) =>
      items?.item_type_values?.types === 'TEXT_INPUT'
        ? arrBody.push(
            status_checklist === false
              ? arrNull[index].length === 1
                ? null
                : arrNull[index]
              : datas[index]?.detail_staging?.fill_columns
          )
        : items?.item_type_values?.types === 'COMBO_BOX'
        ? arrBody.push(
            status_checklist === false
              ? arrNull[index].length === 1
                ? null
        ...
        : items?.item_type_values?.types === 'FILE_UPLOAD'
        ? arrBody.push(
  ...
  useEffect(() => {
    if (pageState.length !== 0) {
      for (let index = 0; index < datas.length; index++) {
        const objIndex = datas.findIndex((obj) => obj?.item_type_values?.types === 'FILE_UPLOAD');
        if (pageState[objIndex]?.fill_columns !== undefined) {
          if (file[0]?.path !== undefined) {
            ...
            status_checklist === false
              ? (pageState[objIndex].fill_columns = null)
              : datas[index]?.detail_staging?.fill_columns;
          }
        }
        // }
  ...
  }, [encodedData, pageState, datas, file]);
  ...
  return (
    <TableContainer className="w-full h-full" component={Paper}>
      {loading === true ? (
        <FuseLoading />
      ) : datas.length !== 0 ? (
          ...
          <TableBody>
            {rows?.map((row, index) => (
              ...
              >
                ...
                {row?.item_type_values?.types === 'TEXT_INPUT' ? (
                  <>
                    <StyledTableCell className="w-10" align="center">
                      {row?.item_type_values?.labels?.map((i, idx) => {
                        return (
                          <TextField
                            disabled={
                              userRoles === 'GUEST' ||
                            ...
                            defaultValue={
                              Array.isArray(row.detail_staging?.fill_columns) === false
                                ? row?.detail_staging?.fill_columns
                                : row.detail_staging?.fill_columns[idx]
                            }
                            label={i}
                          />
                        );
                ...
                ) : row?.item_type_values?.types === 'COMBO_BOX' &&
                  row?.item_type_values?.count_data !== 1 ? (
                  <>
                ...
                ) : row?.item_type_values?.types === 'FILE_UPLOAD' ? (
                  <>
                    <StyledTableCell align="center">
                      <div>
        ...
        <div className="w-full h-full flex justify-center items-center">
          <div className="flex-col justify-center items-center gap-5">
            <Alert severity="warning">
              ...
              <Link className="font-medium" to="/apps/registration/prestaging">
                Prestaging Details Menu.
              </Link>
            </Alert>
          </div>
        </div>
