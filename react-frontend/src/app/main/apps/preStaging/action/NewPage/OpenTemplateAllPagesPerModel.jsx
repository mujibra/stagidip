...
export default function OpenTemplateAllPagesPerModel(props) {
  ...
  const [time_checklist, settime_checklist] = useState(null);
  ...
  const getData = async () => {
    ...
    const response = await axios
      ...
      .then((res) => {
        // console.log(res, 'res');
        settime_checklist(res?.data?.time_checklist);
  ...
  const handleProblem = (e, newValue, index) => {
    if (pageState.length > 0) {
      ...
      const insertState = copyPageState[index];
      const copyObject = { ...insertState };
      copyObject.problem = newValue?.id === undefined ? null : newValue?.id;
      console.log(pageState, 'pageState');
      const newState = pageState.splice(index, 1, copyObject);
      setPageState(pageState);
    }
  ...
  const handleAction = (e, newValue, index) => {
    if (pageState.length > 0) {
      ...
      const insertState = copyPageState[index];
      const copyObject = { ...insertState };
      copyObject.action = newValue?.id === undefined ? null : newValue?.id;
      // console.log(pageState, 'pageState');
      const newState = pageState.splice(index, 1, copyObject);
      setPageState(pageState);
    }
  ...
  const handleRemake = (e, newValue, index) => {
    if (pageState.length > 0) {
      ...
      const insertState = copyPageState[index];
      const copyObject = { ...insertState };
      copyObject.remark = newValue?.id === undefined ? null : newValue?.id;
      // console.log(pageState, 'pageState');
      const newState = pageState.splice(index, 1, copyObject);
      setPageState(pageState);
    }
  };
  const handleAllState = (e, newValue, index, multi = false, idxMulti = 0, name) => {
    if (pageState.length > 0) {
      const copyPageState = [...pageState];
      const insertState = copyPageState[index];
      const copyObject = { ...insertState };
      if (newValue === undefined) {
        if (name === 'VDisplay' || name === 'bios') {
          copyObject.fill_columns[idxMulti] = e.target.value;
        } else {
          copyObject.fill_columns = e.target.value;
        }
      } else if (name === 'bcu') {
        copyObject.fill_columns = newValue?.id === undefined ? null : newValue?.id;
      } else {
        copyObject.fill_columns[idxMulti] = newValue?.id === undefined ? null : newValue?.id;
      }
      // console.log(newValue, 'newValue');
      // console.log(idxMulti, 'idxMulti');
      // console.log(pageState, 'pageState');
      const newState = pageState.splice(index, 1, copyObject);
      setPageState(pageState);
    }
  ...
  useEffect(() => {
    ...
    datas?.map((items, index) =>
      items?.result_detail === 'VDisplay'
        ? arrBody.push(
            status_checklist === false ? [null, null] : datas[index]?.detail_staging?.fill_columns
          )
        : items?.result_detail === 'Dev. 1,2,3,4,5' ||
          items?.result_detail === 'Dev. 1,2,3,4,5,6' ||
          items?.result_detail === 'Dev. 1,2,3,4,5,6,7'
        ? arrBody.push(
            status_checklist === false
              ? [null, null, null, null, null]
              : datas[index]?.detail_staging?.fill_columns
          )
        : items?.result_detail ===
        ...
        : items?.result_detail === 'M/B Bios Version= '
        ? arrBody.push(datas[index]?.detail_staging?.fill_columns)
        : items?.result_detail === 'BCU: BCU20 / BCU24 / BCU53 / BCU54 / BCU'
        ? arrBody.push(datas[index]?.detail_staging?.fill_columns)
        : items?.result_detail === 'attach here the vesion' ||
          items?.result_detail === 'attach here the version information paper'
        ? arrBody.push(
            status_checklist === false ? null : datas[index]?.detail_staging?.fill_columns
          )
        : arrBody.push(null)
    );
  ...
  useEffect(() => {
    if (pageState.length !== 0) {
      for (let index = 0; index < datas.length; index++) {
        if (
          datas[index].result_detail === 'attach here the vesion' ||
          datas[index].result_detail === 'attach here the version information paper'
        ) {
          const objIndex = pageState.findIndex(
            (obj) => obj?.id_checklist_staging === datas[index]?.id
          );
          if (pageState[objIndex]?.fill_columns !== undefined) {
            pageState[objIndex].fill_columns =
              encodedData === null ? datas[index]?.detail_staging?.fill_columns : encodedData;
          }
        }
      }
    }
        ...
        <TableBody>
          {rows?.map((row, index) => (
            ...
            >
              ...
              {row?.result_detail === 'M/B Bios Version= ' ? (
                <>
                  <StyledTableCell className="w-10" align="center">
                    ...
                    <TextField
                      ...
                      onChange={(e, newValue) => {
                        handleAllState(e, newValue, index, true, 0, 'biosVersion');
                        // handleAllState(e, index, true, 0, 'biosVersion');
                      }}
                      defaultValue={
                  ...
                  <StyledTableCell align="center">
                    <div className="flex gap-5 justify-between">
                      <TextField
                        ...
                        // }}
                        onChange={(e, newValue) => {
                          handleAllState(e, newValue, index, true, 0, 'VDisplay');
                          // handleAllState(e, index, true, 0, 'biosVersion');
                        }}
                        label="V"
                      />
                      <TextField
                        ...
                        // }}
                        onChange={(e, newValue) => {
                          handleAllState(e, newValue, index, true, 1, 'VDisplay');
                        }}
                        label="Display"
                      />
                    </div>
              ...
              ) : row?.result_detail === 'Dev. 1,2,3,4,5' ||
                row?.result_detail === 'Dev. 1,2,3,4,5,6' ||
                row?.result_detail === 'Dev. 1,2,3,4,5,6,7' ? (
                <>
                  <StyledTableCell align="center">
                    <div className="flex gap-10 justify-between">
                      ...
                      <div>
                        <Autocomplete
                          ...
                          // }}
                          onChange={(e, newValue) => {
                            handleAllState(e, newValue, index, true, 0, 'dev');
                          }}
                          // id="combo-box-demo"
                          renderInput={(params) => (
                            <TextField
                      ...
                      <div>
                        <Autocomplete
                          ...
                          // }}
                          onChange={(e, newValue) => {
                            handleAllState(e, newValue, index, true, 1, 'dev');
                          }}
                          defaultValue={
                            row.detail_staging?.fill_columns === null
                              ? dev2
                      ...
                      <div>
                        <Autocomplete
                          ...
                          onChange={(e, newValue) => {
                            handleAllState(e, newValue, index, true, 2, 'dev');
                          }}
                          disablePortal
                          // id="combo-box-demo"
                          options={DataDenomination}
                      ...
                      <div>
                        <Autocomplete
                          ...
                          // }}
                          onChange={(e, newValue) => {
                            handleAllState(e, newValue, index, true, 3, 'dev');
                          }}
                          disablePortal
                          // id="combo-box-demo"
                          options={DataDenomination}
                      ...
                      <div>
                        <Autocomplete
                          ...
                          // }}
                          onChange={(e, newValue) => {
                            handleAllState(e, newValue, index, true, 4, 'dev');
                          }}
                          disablePortal
                          // id="combo-box-demo"
                          options={DataDenomination}
              ...
              ) : row?.result_detail === 'BCU: BCU20 / BCU24 / BCU53 / BCU54 / BCU' ? (
                <>
                  <StyledTableCell component="th" scope="row">
                    <div className="flex flex-col gap-5">
                      <Autocomplete
                        ...
                        //   }}
                        onChange={(e, newValue) => {
                          handleAllState(e, newValue, index, true, 0, 'bcu');
                          // handleAllState(e, index, true, 0, 'biosVersion');
                        }}
                        disablePortal
                        // id="combo-box-demo"
              ...
              ) : row?.result_detail === 'attach here the vesion' ||
                row?.result_detail === 'attach here the version information paper' ? (
                <>
                  <StyledTableCell align="center">
                    <div>
                  ...
                  <StyledTableCell align="center">
                    ...
                    <div className="flex flex-col gap-5">
                      <TextField
                        ...
                        // }}
                        onChange={(e, newValue) => {
                          handleAllState(e, newValue, index, true, 0, 'bios');
                        }}
                        defaultValue={
                          row.detail_staging?.fill_columns === null
                            ? hour
                      ...
                      <TextField
                        ...
                        // }}
                        onChange={(e, newValue) => {
                          handleAllState(e, newValue, index, true, 1, 'bios');
                        }}
                        defaultValue={
                          row.detail_staging?.fill_columns === null
                            ? minute
                      ...
                      <TextField
                        ...
                        // }}
                        onChange={(e, newValue) => {
                          handleAllState(e, newValue, index, true, 2, 'bios');
                        }}
                        defaultValue={
                          row.detail_staging?.fill_columns === null
                            ? second
