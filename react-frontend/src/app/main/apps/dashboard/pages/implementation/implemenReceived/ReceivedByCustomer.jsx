...
import MachineReceivedFilter from '../../filterComponent/MachineReceivedFilter';
...
const ReceivedByCustomer = (props) => {
  ...
  const getData = async () => {
    ...
    const response = await axios
      .get(
        `${api}getMachineReceivedByCustomer`,
        {
          headers: {
            Authorization: `Bearer ${getAccessToken}`,
          },
  ...
  const dataLabel = data?.map((item) => item?.customer_name);
  const TotalReceived = data?.map((item) => item?.quantity);
  const dataTotalReceived = TotalReceived?.map((item) => {
    return item === null || item === '0' || item === undefined ? 0 : parseInt(item);
  });
  const totalMesin = dataTotalReceived.reduce((acc, curr) => acc + curr, 0);
  // console.log(filteredData, 'data');
  // console.log(dataLabel, 'dataLabel');
  // console.log(dataTotalReceived, 'dataTotalReceived');
  ...
  const handleDataPointSelection = (event, chartContext, configKlik) => {
    ...
    const selectedDataValue = dataTotalReceived[selectedDataIndex];
  ...
  const state = {
    series: dataTotalReceived?.length !== 0 ? dataTotalReceived : [],
    options: {
      chart: {
        type: 'donut',
        events: {
  ...
  // useEffect(() => {}, [state, statePercent, showPercent]);
      ...
      <Card className="gap-20 ml-10 mr-10 mt-20">
        <div className='flex justify-between'>
          <Typography className="m-20 text-17" color="textSecondary">
            <b>Machine Received by Customer</b>
          </Typography>
          <MachineReceivedFilter
            month={month}
            setMonth={setMonth}
            year={year}
            setYear={setYear}
                    ...
                    <div className="px-10 py-4 text-xs md:text-sm">
                      ...
                      <p>
                        ...
                        <span className="ml-10 font-semibold">
                          {selectedDataPercent.percentage}%
                        </span>
                      </p>
                    </div>
                  )}
...
export default ReceivedByCustomer;
