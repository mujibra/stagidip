...
const ReceivedByType = (props) => {
  ...
  const combineDuplicateDatas = () => {
    ...
    data.forEach((item) => {
      ...
      if (combinedDatas.hasOwnProperty(item?.type_mesin)) {
        combinedDatas[item?.type_mesin].quantity += item?.quantity;
      } else {
  ...
  const dataTotalActivated = combinedDatas.map((item) => parseInt(item?.quantity));
