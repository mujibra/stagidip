...
import MachineDeliveryFilter from '../../filterComponent/MachineDeliveryFilter';
...
const DeliveryByType = (props) => {
  ...
  const getData = async () => {
    ...
    const response = await axios
      .get(`${api}getMachineDeliveryByType`, {
        headers: {
          Authorization: `Bearer ${getAccessToken}`,
        },
        params: {
  ...
  //       combinedDatas[item?.type_mesin].total_kirim += item?.total_kirim;
  //     } else {
  ...
  const dataLabel = data?.map((item) => item?.type_model);
  const dataTotalDelivery = data?.map((item) => item?.total_mesin_delivered);
  ...
  const series = [
    {
      ...
      data: dataTotalDelivery?.length !== 0 ? dataTotalDelivery : [],
    },
  ];
      ...
      <Card className="gap-20 ml-10 mr-10 mt-20">
        <div className='flex justify-between'>
          <Typography className="m-20 text-17" color="textSecondary">
            <b>Machine Delivery by Type</b>
          </Typography>
          <MachineDeliveryFilter
            month={month}
            setMonth={setMonth}
            year={year}
            setYear={setYear}
