...
import OldMachineTable from './OldMachineTable';
...
export default function NewMachineList(props) {
  ...
  // const selectOldMachineByIds = useSelector(selectOldMachineById);
  ...
  const urlPrev = `/apps/stagging/oldMachine/${idPo}-${idMesinPrev}`;
  ...
  let url;
  if (valuePagin === null) {
    url = `/apps/stagging/oldMachine/${idPo}-${idMesin}`;
  } else {
    url = `/apps/stagging/oldMachine/${idPo}-${valuePagin}`;
  }
  const handleOnClick = () => {
    navigate(url);
    setValuePagin(null);
  ...
  const downloadPDF = () => {
    ...
    doc.save(
      `Registrasi Model Old Machine No PO : ${getIdMesin?.no_po} ${moment().format('LL')}.pdf`
    );
  };
  ...
  function exportExcel() {
    ...
    const sheet = workbook.addWorksheet('Registrasi Model');
    const worksheet = workbook.getWorksheet('Registrasi Model');
    ...
    // worksheet.getCell('A3').value = `Registrasi Mesin model : ${
    //   props.dataDetailOrder?.mesin.type === null
    //     ? '-'
    //     : props.dataDetailOrder?.mesin.type
    // }`;
    ...
    (async () => {
      ...
      saveAs(
        blob,
        `Registrasi Model Old Machine No PO : ${getIdMesin?.no_po} ${moment().format(
          'LL'
        )}${fileExtension}`
      );
    })();
        ...
        <>
          ...
          <div className="my-10 grid md:grid-flow-row-dense sm:justify-between  w-full ">
            <div className="w-full flex justify-between ">
              <div className="gap-5 flex justify-between md:w-full">
            ...
            {/* Button */}
            <div className="flex flex-row-reverse p-12 sm:grid-rows-6 sm">
              <Button
                variant="contained"
                disabled={
                  userRoles === 'GUEST' ||
          ...
          {/* prev */}
          <div className="flex flex-row-reverse sm:w-full p-full justify-between">
