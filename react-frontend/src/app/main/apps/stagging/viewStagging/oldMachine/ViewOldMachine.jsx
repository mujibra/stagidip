...
import HandleExportExcel from './HandleExportExcel';
import TableViewOldwMachine from './TableViewOldwMachine';

export const ViewOldMachine = () => {
  ...
  const getData = async () => {
    ...
    const response = await axios
      .get(`${api}stagging/oldMachine`, config)
      .then((res) => {
        // console.log(res, 'RES');
  ...
  const downloadPDF = () => {
    ...
    doc.text(`List Staging Old Machine Tanggal ${moment().format('LL')}`, 20, 20);
    const index = 0;
    autoTable(doc, {
      theme: 'striped',
      head: [
    ...
    doc.save(`Download List Staging Old Machine ${moment().format('LL')}.pdf`);
  };
  // };
  ...
  //   axios
  //     .get(
  //       `${process.env.REACT_APP_API_URL_API_DATINDO_LOCAL}stagging/oldMachine`,
  //       config
  //     )
  //     .then(res => {
  //       setLoading(false);
  ...
  return (
    <div>
      <FusePageCarded
        ...
        content={
          <div>
            <TableViewOldwMachine loading={loading} data={data} />
          </div>
        }
      />
    </div>
