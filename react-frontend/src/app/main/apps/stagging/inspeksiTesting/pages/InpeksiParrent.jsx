...
import TableInpeksi from './TableInpeksi';
...
const InpeksiParrent = (props) => {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`List Pre Loading ${moment().format('LL')}`, 20, 20);
    const index = 0;
    autoTable(doc, {
      theme: 'striped',
      head: [
    ...
    doc.save(`Download Pre Loading ${moment().format('LL')}.pdf`);
  };
  // };
        ...
        content={
          <FuseAnimate animation="transition.slideLeftIn" delay={100}>
            <div className="p-16 sm:p-24 max-w-full items-left">
              <div>
                <TableInpeksi setLoading={setLoading} loading={loading} data={data} />
              </div>
            </div>
          </FuseAnimate>
        }
