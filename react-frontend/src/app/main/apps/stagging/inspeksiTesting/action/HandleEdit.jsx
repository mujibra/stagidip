...
export default function HandleEdit(props) {
  ...
  const rows = data?.map((item, index) =>
    createData(item?.id, item?.general_desc, item?.detail_inspeksi)
  );
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      } else if (dataById?.model?.name === 'CS KIOS') {
        arr.push({
          ...body,
          id_inspeksi: data[i]?.id,
          position: data[i]?.detail_inspeksi?.position,
  ...
  return (
    <Paper sx={{ width: '100%' }}>
      <div className="w-full flex justify-end m-5 mr-10">
        {/* <Button className="mr-32" variant="contained" onClick={handleEdit}>
          Edit
        </Button> */}
      </div>
