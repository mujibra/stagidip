...
function createData(id, status_desc) {
  return {
    id,
    status_desc,
  };
    ...
    <TableHead className="bg-blue-50">
      <TableRow>
        {headCells.map((headCell) => (
          <TableCell
            key={headCell.id}
            align={headCell.id === 'no' || headCell.id === 'status_desc' ? 'center' : 'center'}
            // align="center"
...
export default function TableStatusPo(props) {
  const { body } = props;
  const { setBody } = props;
  ...
  const filteredData = props?.data
    ? props?.data.filter((el) => {
        if (searched) {
          return el?.status_desc?.toLowerCase().includes(searched.toLowerCase());
        }
        return el;
      })
    : [];
  ...
  // const bodyEdit = {
  //   id: dataEdit.id,
  //   status_desc: dataEdit.status_desc,
  ...
  const handleEdit = async (id) => {
    ...
    setLoading(true);
    const response = await axios
      .put(`${api}ubahStatusPo/${id}`, newBody, config)
      .then((res) => {
        setLoading(false);
        dispatch(
          showMessage({
