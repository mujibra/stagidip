...
function createData(id, gudang_desc, alamat) {
  return {
    id,
    gudang_desc,
    alamat,
    ...
    <TableHead className="bg-blue-50">
      <TableRow>
        ...
        {headCells.map((headCell) => (
          <TableCell
            key={headCell.id}
            align={headCell.id === "action" ? "center" : "center"}
            // align="center"
...
export default function TableWarehouse(props) {
  ...
  const filteredData = props?.data
    ? props?.data.filter((el) => {
        if (searched) {
          return (
            el?.gudang_desc?.toLowerCase().includes(searched.toLowerCase()) ||
            el?.alamat?.toLowerCase().includes(searched.toLowerCase())
          );
        }
        return el;
      })
  ...
  const bodyEdit = {
    // id: dataById?.id,
    gudang_desc: dataById?.gudang_desc,
    alamat: dataById?.alamat,
  };
  ...
  const handleEdit = async (id) => {
    const newBody = {
      gudang_desc: dataEdit?.gudang_desc,
    ...
    const response = await axios
      .put(`${api}master-gudang/${id}`, newBody, config)
      .then((res) => {
        setLoading(false);
        dispatch(
          showMessage({
          ...
          <div>
            ...
            <DialogContent>
              <DialogContentText id="alert-dialog-description">
                {`Sure to delete data ${row?.gudang_desc}?`}
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button
  ...
  //   props.setLoading(true);
  //   const response = await axios
  //     .put(`${api}master-gudang/${id}`, bodyEdit, {
  //       headers: {
  //         'Content-Type': 'application/json',
  //         Authorization: `Bearer ${getAccessToken}`,
  //       },
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell
                          ...
                          className={classes?.tableCell}
                        >
                          {row?.gudang_desc === "" ? "-" : row?.gudang_desc}
                        </TableCell>
                        <TableCell
                          align="center"
                          className={classes?.tableCell}
