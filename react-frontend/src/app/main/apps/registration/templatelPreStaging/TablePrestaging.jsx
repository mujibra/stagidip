...
function createData(id, name, mesin, id_mesin) {
  return {
    id,
    name,
    mesin,
    ...
    <TableHead className="bg-blue-50">
      <TableRow>
        {headCells.map((headCell) => (
          <TableCell
            key={headCell.id}
            // className={headCell.id === 'no' || headCell.id === 'item' ? 'ml-5' : ''}
...
export default function TablePrestaging(props) {
  const { body } = props;
  const { setBody } = props;
  const dispatch = useDispatch();
  const payloadStateBody = useRef(null);
  ...
  const propsFromParent = (stateBody) => {
    payloadStateBody.current = stateBody;
  };
  // console.log(payloadStateBody.current, 'payloadStateBody.current');

  const getData = async () => {
    setLoading(true);
    const response = await axios
      .get(
        `${process.env.REACT_APP_API_URL_API_DATINDO_LOCAL}dataTableChecklist/${dataEdit?.id_mesin}/${dataEdit?.id}`,
        config
      )
      .then((res) => {
  ...
  const handleSubmit = async (id) => {
    ...
    const response = await axios
      .post(
        `${process.env.REACT_APP_API_URL_API_DATINDO_LOCAL}mst-checkliststaging/${dataEdit?.id}`,
        payloadStateBody.current,
        config
      )
      .then((res) => {
  ...
  const HandleDelete = async (id, row) => {
    ...
    const response = await axios
      ...
      .then((res) => {
        ...
        dispatch(
          showMessage({
            message: `Data ${row?.name} Successfully Deleted!`, // text or html
            autoHideDuration: 6000, // ms
            anchorOrigin: {
              vertical: 'top', // top bottom
              horizontal: 'center', // left center right
        ...
        >
          ...
          <DialogActions>
            ...
            <Button
              ...
              //   dataEdit?.label === '' ||
              //   dataEdit?.divisi === '' ||
              //   dataEdit?.name.length === 0
              // }
              variant="contained"
              startIcon={<PlaylistAddIcon />}
            >
              <div className="hidden md:contents" onClick={handleSubmit}>
                Save
              </div>
            </Button>
          </DialogActions>
                  ...
                  .map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="center" className={classes?.tableCell}>
                          {row?.mesin?.type === null ? '-' : row?.mesin?.type}
                        </TableCell>
                            ...
                            {user_info[0]?.roles !== 'SUPERVISOR' && (
                              <div>
                                <IconButton
                                  ...
                                  onClick={(e) => HanldleClickNotifDelete(row?.id, row)}
                                  color="error"
                                  className=""
                                >
                                  <DeleteIcon />
