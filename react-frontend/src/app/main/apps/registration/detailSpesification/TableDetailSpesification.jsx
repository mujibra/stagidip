...
function createData(id, parent, data_type) {
  return {
    id,
    parent,
    data_type,
    ...
    <TableHead className="bg-blue-50">
      <TableRow>
        {headCells.map((headCell) => (
          <TableCell
            key={headCell.id}
            align={headCell.id === 'no' || headCell.id === 'item' ? 'left' : 'center'}
            // align="center"
            // width={20}
...
export default function TableDetailSpesification(props) {
  const { body } = props;
  const { setBody } = props;
  ...
  const [getDisableIsSame, setgetDisableIsSame] = useState(false);
  ...
  const arrEdit = dataById?.data_type;
  const arrValueEdit = getBody;
  const arrPushEdit = [];
  const arrPushValueEdit = [];
  ...
  useEffect(() => {
    for (let index = 0; index < arrEdit?.length; index++) {
      arrPushEdit.push(dataById?.data_type[index]?.name);
    }
    if (getBody?.length !== 0) {
      for (let index = 0; index < getBody?.length; index++) {
        arrPushValueEdit.push(getBody[index]?.type);
      }
      // console.log(arrPushEdit, 'arrPushEdit');
      // console.log(arrPushValueEdit, 'arrPushValueEdit');
      // console.log(arraysAreEqual(arrPushEdit, arrPushValueEdit));
      setgetDisableIsSame(arraysAreEqual(arrPushEdit, arrPushValueEdit));
    }
  }, [getBody]);
  // console.log(getDisableIsSame, 'getDisableIsSame');
  // console.log(arrPushValueEdit, 'getBody?.data_type');

  // Memeriksa apakah semua nilai dalam dua array sama
  ...
  const handleEdit = async (id) => {
    ...
    const response = await axios
      .put(`${api}updateParentType/${id}`, { type_atm: getBody }, config)
      .then((res) => {
        setLoading(false);
        dispatch(
          showMessage({
  ...
  const HandleDelete = async (id, e) => {
    ...
    setLoading(true);
    const response = await axios
      .delete(`${api}removeParentType/${id}`, {
        headers: {
          Authorization: `Bearer ${getAccessToken}`,
        },
      })
          ...
          <div>
            ...
            <DialogContent>
              <DialogContentText id="alert-dialog-description">
                {`Sure to delete data ${row?.parent}?`}
              </DialogContentText>
            </DialogContent>
            <DialogActions>
  ...
  const truncateArray = (arr, maxLength) => {
    ...
    return `${truncatedArr.substring(0, maxLength)}...(${arr.length} Type)`;
  };
  ...
  useEffect(() => {
    if (searched === '') {
      ...
      const filteredData = props?.data?.filter((row) =>
        row?.parent?.toLowerCase()?.includes(searched?.toLowerCase())
      );
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              <HandleEditDetail
                ...
                getDisableIsSame={getDisableIsSame}
              />
            </DialogContentText>
          </DialogContent>
          <DialogActions>
