...
function createData(id, no_po_master, tgl_po, customer) {
  return {
    id,
    no_po_master,
    tgl_po,
...
export default function TableRegistPO(props) {
  const { body } = props;
  const { setBody } = props;
  ...
  const filteredData = props?.data
    ? props?.data.filter((el) => {
        if (searched) {
          return (
            el?.no_po_master?.toLowerCase().includes(searched.toLowerCase()) ||
            moment(el?.tgl_po)
              .format('DD MMM YYYY')
              .toLowerCase()
  ...
  const bodyEdit = {
    // id: dataById?.id,
    no_po_master: dataById?.no_po_master,
    tgl_po: moment(dataById.tgl_po).format('YYYY-MM-DD HH:mm:ss'),
    customer: dataById?.bank_desc?.id,
  };
  ...
  const handleEdit = async (id) => {
    const newBody = {
      tgl_po: moment(dataEdit?.tgl_po).format('YYYY-MM-DD HH:mm:ss'),
      no_po_master: dataEdit?.no_po_master,
      id_customer: dataEdit?.customer?.id,
      // customer: dataEdit?.customer,
    };
    ...
    const response = await axios
      .put(`${api}master-po/${id}`, newBody, config)
      .then((res) => {
        setLoading(false);
        dispatch(
          showMessage({
          ...
          <div>
            ...
            <DialogContent>
              <DialogContentText id="alert-dialog-description">
                {`Sure to delete data ${row?.no_po_master}?`}
              </DialogContentText>
            </DialogContent>
            <DialogActions>
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              <HandleEditDetail
                ...
                // wrapperBodyEdit={wrapperBodyEdit}
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left" className={classes?.tableCell}>
                          {row?.no_po_master === '' ? '-' : row?.no_po_master}
                        </TableCell>
                        <TableCell align="center" className={classes?.tableCell}>
                            ...
                            <div className="flex justify-center">
                              ...
                              {/* <div>
                                <IconButton
                                  // onClick={(e) => HandleDelete(row.id,row.partNumber, e)}
                                  onClick={(e) =>
                                    HanldleClickNotifDelete(row.id, row.partNumber, e)
                                  }
                                  color="error"
                                  className=""
                                >
