      ...
      <TableRow>
        ...
        {headCells.map((headCell) => (
          <TableCell
            key={headCell.id}
            align={
              headCell.id === 'name' ? 'left' : 'center' || headCell.id === 'no' ? 'left' : 'center'
            }
            // align="center"
...
export default function TableModel(props) {
  ...
  const handleEdit = async (id) => {
    ...
    props.setLoading(true);
    // const token = '50|HMnOD4yoIX3LV96WEmtWMtwK9qM6oALK2IAk4hCF';
    const response = await axios
      .put(`${api}master-model/${id}`, bodyEdit, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${getAccessToken}`,
        },
      })
      // .put(`${api}/machine/${id}`, bodyEdit)
      .then((res) => {
        // console.log(res);
        setDataEdit(res.data?.data);
        props.setLoading(false);
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <div className="flex justify-center">
                            <div>
                              <IconButton
                                onClick={() => handleClickOpens(row.id)}
                                color="info"
                                className=""
                              >
                                <EditIcon />
