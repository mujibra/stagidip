...
function createData(id, bank_desc, address) {
  return {
    id,
    bank_desc,
    address,
...
export default function TableCustomer(props) {
  ...
  const filteredData = props?.data
    ? props?.data.filter((el) => {
        if (searched) {
          return (
            el?.bank_desc?.toLowerCase().includes(searched.toLowerCase()) ||
            el?.address?.toLowerCase().includes(searched.toLowerCase())
          );
        }
        return el;
      })
  ...
  const handleEdit = async (id) => {
    ...
    const response = await axios
      .put(`${api}master-customer/${id}`, newBody, config)
      .then((res) => {
        setLoading(false);
        dispatch(
          showMessage({
          ...
          <div>
            ...
            <DialogContent>
              <DialogContentText id="alert-dialog-description">
                {`Sure to delete data ${row?.bank_desc}?`}
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button
  ...
  //   props.setLoading(true);
  //   const response = await axios
  //     .put(`${api}master-customer/${id}`, bodyEdit, {
  //       headers: {
  //         'Content-Type': 'application/json',
  //         Authorization: `Bearer ${getAccessToken}`,
  //       },
