...
import TableSummaryNewMachine from './TableSummaryNewMachine';
...
const SummaryNewMachineParent = (props) => {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`List Summary New Machine on ${moment().format('LL')}`, 20, 20);
    const index = 0;
    autoTable(doc, {
      theme: 'striped',
    ...
    doc.save(`Summary New Machine ${moment().format('LL')}.pdf`);
  };
            ...
            <div className="flex items-left mt-10 ml-20 w-1/2 flex-col md:flex-row md:items-center md:mt-0">
              {userRoles !== 'GUEST_RELATED' && (
                <div className="w-full flex">
                  ...
                  <div className="ml-10">
                    <HandleExportExcel datasLoop={datasLoop} listPO={id_po} />
                  </div>
                </div>
              )}
            </div>
...
export default SummaryNewMachineParent;
