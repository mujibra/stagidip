...
const SummaryWarehouseParent = (props) => {
  ...
  const [sumQuantityBeforeDelvivery, setsumQuantityBeforeDelvivery] = useState(0);
  const [sumQuantityDelivery, setsumQuantityDelivery] = useState(0);
  ...
  useEffect(() => {
    ...
    if (!isUnmout) {
      axios
        .get(
          `${api}getWarehouseSummary/${nama_gudang === '' ? null : nama_gudang}/${
            customer === '' ? null : customer
          }/${models === '' ? null : models}/${
            styles === undefined || styles === '' ? null : styles
          }/${status_mesin === '' || status_mesin === 'All' ? null : status_mesin}/${
            process_wr === '' || process_wr === 'All' ? null : process_wr
        ...
        .then((res) => {
          ...
          res?.data.data.map((row, index) => {
            totalBalance.push(row?.qty_before_delivery - row?.qty_delivery);
          });
          const initialValue = 0;
          const sumWithInitial = totalBalance.reduce(
            (accumulator, currentValue) => accumulator + currentValue,
            initialValue
          );
          setData(res?.data?.data);
          setsumQuantityBeforeDelvivery(
            (res?.data?.data).reduce((total, obj) => total + obj?.qty_before_delivery, 0)
          );
          setsumQuantityDelivery(
            (res?.data?.data).reduce((total, obj) => total + obj?.qty_delivery, 0)
          );
          setsumBalance(sumWithInitial);
          setLoading(false);
  ...
  const datFilter = data.filter((datax) => datax.qty_before_delivery - datax?.qty_delivery > 0);

  // console.log(datFilter, 'datFilter');

  const datas = datFilter?.map((item, index) =>
    createData(
      // item?.id,
      index + 1,
      ...
      item?.styles === null ? '-' : item?.styles,
      item?.qty_before_delivery,
      item?.qty_delivery,
      item?.qty_before_delivery - item?.qty_delivery,
      item?.status_mesin === '' ? '-' : item?.status_mesin,
  ...
  for (let index = 0; index < datasLoop.length; index++) {
    if (datasLoop.length !== 0) {
      datasLoop[index].index = index + 1; // Mengatur nomor urut
  ...
  const downloadPDF = () => {
    ...
    doc.text(`List Summary Warehouse on ${moment().format('LL')}`, 20, 20);
    doc.setFontSize(10);
    doc.text(
      `Sum Quantity Before Delivery  : ${
        sumQuantityBeforeDelvivery === null || '' || undefined || NaN
          ? '-'
    ...
    doc.text(
      `Sum Quantity Delivery     : ${
        sumQuantityDelivery === null || '' || undefined || NaN ? '-' : sumQuantityDelivery
      }`,
      20,
      55
    );
    ...
    doc.save(`Download Summary Warehouse ${moment().format('LL')}.pdf`);
  };
                    ...
                    <FuseAnimate animation="transition.slideLeftIn" delay={100}>
                      <div className="ml-10">
                        <HandleExportExcel
                          ...
                          datasLoop={datasLoop}
                          listPO={id_po}
                          sumQuantityBeforeDelvivery={sumQuantityBeforeDelvivery}
                          sumQuantityDelivery={sumQuantityDelivery}
...
export default SummaryWarehouseParent;
