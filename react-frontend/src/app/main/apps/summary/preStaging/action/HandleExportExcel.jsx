...
const HandleExportExcel = (props) => {
  ...
  function exportExcel() {
    ...
    const sheet = workbook.addWorksheet('SUMMARY PRE STAGING');
    const worksheet = workbook.getWorksheet('SUMMARY PRE STAGING');

    /* TITLE */
    worksheet.mergeCells('A1', 'G1');
    worksheet.getCell('A1').value = 'SUMMARY PRE STAGING';
    ...
    worksheet.getCell('D3').value = `${'SUM TOTAL:' + ' '}${
      sumTotal === null || sumTotal === undefined || sumTotal === NaN ? '-' : sumTotal
    }`;
    ...
    worksheet.getCell('F6').value = 'PART NUMBER';
    ...
    (async () => {
      ...
      saveAs(blob, `Summary Pre Staging ${moment().format('LL')}${fileExtension}`);
    })();
  }
