...
function createData(no_po, customer, types, model, pn_system, total) {
  return {
    no_po,
    customer,
    types,
...
export default function SummaryWarehouseParent(props) {
  ...
  const datas = props.data?.map((item, index) =>
    createData(item?.no_po, item?.customer, item?.types, item?.model, item?.pn_system, item?.total)
  );
