...
import TableSummaryPreStaging from './TableSummaryPreStaging';
// import  HandleExportExcel  from './action/HandleExportExcel';
...
const SummaryPreStagingParent = (props) => {
  ...
  useEffect(() => {
    ...
    if (!isUnmout) {
      axios
        .get(
          `${api}getPreStagingSummary/${customer === '' ? null : customer}/${
  ...
  function createData(no, no_po, customer, types, model, pn_system, total) {
    return {
      no,
      no_po,
      customer,
  ...
  const downloadPDF = () => {
    ...
    doc.text(`List Summary Pre Staging Tanggal ${moment().format('LL')}`, 20, 20);
    doc.setFontSize(10);
    doc.text(
      `Sum Total  : ${sumTotal === null || '' || undefined || NaN ? '-' : sumTotal}`,
      20,
      40
    );
    const index = 0;
    autoTable(doc, {
      theme: 'striped',
      margin: { top: 65 },
      head: [['N0', 'No PO', 'Customer', 'Type', 'Model', 'PartNumber System', 'Total']],
    ...
    doc.save(`Download Summary Pre Staging ${moment().format('LL')}.pdf`);
  };
...
export default SummaryPreStagingParent;
