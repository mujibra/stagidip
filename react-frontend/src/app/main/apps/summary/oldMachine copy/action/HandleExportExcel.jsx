...
const HandleExportExcel = (props) => {
  ...
  function exportExcel() {
    ...
    const sheet = workbook.addWorksheet('SUMMARY STATUS DELIVERY');
    const worksheet = workbook.getWorksheet('SUMMARY STATUS DELIVERY');

    /* TITLE */
    worksheet.mergeCells('A1', 'G1');
    worksheet.getCell('A1').value = 'SUMMARY STATUS DELIVERY';
    worksheet.getCell('A1').alignment = { horizontal: 'left' };
    worksheet.mergeCells('A3', 'G3');
    worksheet.getCell('A3').value = `${'NO PO:' + ' '}${
      listPO === null || listPO === undefined ? 'All PO' : listPO?.no_po ? listPO?.no_po : 'All PO'
    }`;
    ...
    worksheet.getCell('D6').value = 'SN MACHINE';
    worksheet.getCell('D6').alignment = { horizontal: 'center' };

    worksheet.getCell('E6').value = 'ARRIVAL DATE';
    worksheet.getCell('E6').alignment = { horizontal: 'center' };

    worksheet.getCell('F6').value = 'WAREHOUSE';
    ...
    datas.forEach(function (data, index) {
      worksheet.addRow({
        ...
        data_d: data?.mesin,
        data_e: moment(data?.tgl_perkiraan_tiba).format('ll'),
        data_f: data?.gudang,
      });
    });
    ...
    (async () => {
      ...
      saveAs(blob, `Summary of Status Delivery ${moment().format('LL')}${fileExtension}`);
    })();
  }
