...
import TableSummaryStatusDelivery from './TableSummaryStatusDelivery';
// import  HandleExportExcel  from './action/HandleExportExcel';
...
const StatusDeliveryParent = (props) => {
  ...
  useEffect(() => {
    ...
    if (!isUnmout) {
      axios
        .get(
          `${api}get-status-delivery/${body?.id_po?.id === undefined ? null : body?.id_po?.id}/${
            body?.snMesin
          }/${body?.customer}/${body?.nama_gudang}/${
  ...
  function createData(no, no_po, customer, mesin, tgl_perkiraan_tiba, gudang) {
    return {
      no,
      no_po,
      customer,
  ...
  const datas = data?.map((item, index) =>
    createData(
      ...
      // item?.id,
      item?.detailPo?.no_po,
      item?.detailPo?.customer?.bank_desc,
      item?.sn_mesin,
      item?.tgl_perkiraan_tiba,
      item?.detailPo?.gudang?.gudang_desc
    )
  );
  ...
  const downloadPDF = () => {
    ...
    doc.text(`List Summary of Status Delivery on ${moment().format('LL')}`, 20, 20);
    const index = 0;
    autoTable(doc, {
      theme: 'striped',
      head: [['No', 'No PO', 'Customer', 'SN Machine', 'Arrival Date', 'Warehouse']],
    ...
    doc.save(`Summary of Status Delivery ${moment().format('LL')}.pdf`);
  };
  ...
  return (
    <div>
      <FusePageCarded
        ...
        header={
          <div>
            <div className="flex my-10 flex-1 w-full items-center justify-between">
...
export default StatusDeliveryParent;
