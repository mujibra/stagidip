# StagiDIP API Endpoint Inventory (Current Code State)

Generated from `app/api/**/route.ts` to help QA validate actual implemented routes.

- Total route handlers: **189**
- Base prefix for all endpoints: **`/api`**
- Dynamic path params are shown in `[brackets]` exactly as in source.
- `Body` is marked as `JSON` when `.json()` parsing is detected in the handler file.

## QA Usage Notes

1. Validate method/path first (expect 404/405 for invalid combinations).
2. For routes with `Body: JSON`, send `Content-Type: application/json`.
3. For routes with query params, test with and without optional query keys.
4. Cross-check response shape directly from the route file listed in the last column.

## `addNewDivisi`

| Methods | Path | Path Params | Query Params (detected) | Body | Source |
|---|---|---|---|---|---|
