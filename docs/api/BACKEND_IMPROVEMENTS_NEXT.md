...
| P1 | Add API contract drift gate in CI | Catches docs/runtime mismatch early | PR pipeline runs docs contract checks (`npm run docs:check-contract-drift`) and blocks unreviewed contract drift | _TBD_ | `in-progress (90%)` |
| P1 | Expand smoke API coverage for critical flows | Increases release confidence across integration paths | `npm run qa:smoke` covers at least one happy-path + one error-path for each critical chain (PO -> checklist -> status delivery; pre-staging -> checklist -> approval) | _TBD_ | `in-progress (96%)` |
...
- P1 — Add API contract drift gate in CI: **90%**
- P1 — Expand smoke API coverage for critical flows: **96%**
...
Next steps to close to 100%:
- Configure stable fixture secrets in CI (`QA_EMAIL`, `QA_PASSWORD`, `QA_PO_ID`, `QA_MESIN_ID`, `QA_DIVISI_ID`, `QA_MV400_PO_ID`, `QA_MV400_MESIN_ID`, `QA_STATUS_ID_PO`, `QA_STATUS_SN_MESIN`) to run full chain assertions on every API PR.
